...
const Navbar = () => {
    ...
    const { user, navigate, isOwner, setShowBilliardReg } = useAppContext();
            ...
            <div className="hidden md:flex items-center gap-4 lg:gap-8">
                ...
                {user && (
                    <button
                        ...
                        onClick={() => isOwner ? navigate('/owner') : setShowBilliardReg(true)}
                    >
                        {isOwner ? 'Dashboard' : 'List Your Billiard'}
                    </button>
                )}
            </div>
            ...
            <div className={`fixed top-0 left-0 w-full h-screen bg-white text-base flex flex-col md:hidden items-center justify-center gap-6 font-medium text-gray-800 transition-all duration-500 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                {user && (
                    <button className="border px-4 py-1 text-sm font-light rounded-full cursor-pointer transition-all"
                        onClick={() => { setIsMenuOpen(false); isOwner ? navigate('/owner') : setShowBilliardReg(true); }}>
                        {isOwner ? 'Dashboard' : 'List Your Billiard'}
                    </button>
                )}
                {!user && (
