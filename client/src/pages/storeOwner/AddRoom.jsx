...
const AddRoom = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    if (!inputs.roomType || !inputs.pricePerNight || !inputs.amenities || !Object.values(images).some(image => image)) {
      toast.error("Harap isi semua detailnya")
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <form onSubmit={onSubmitHandler}>
      <Title align='left' font='outfit' title='Add Table' subTitle='Fill in the details carefully and accurate table details, pricing, and amenities, to enchance the user booking experience.' />
      ...
      <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
        <div className='flex-1 max-w-48'>
          <p className='text-gray-800 mt-4'>Brand Table</p>
          <select value={inputs.roomType} onChange={e => setInputs({ ...inputs, roomType: e.target.value })}
            className='border opacity-70 border-gray-300 mt-1 rounded p-2 w-full'>
            <option value="">Select Brand Table </option>
            <option value="XINGJUE">XINGJUE</option>
            <option value="MR.SUNG">MR.SUNG</option>
            <option value="MAJESTIX">MAJESTIX</option>
